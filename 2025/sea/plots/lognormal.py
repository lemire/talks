...
# Parameters for the log-normal distribution (mu and sigma for the underlying normal)
mu = 0.0      # Mean of the underlying normal distribution
sigma = 1.0   # Standard deviation of the underlying normal distribution
N = 10        # Number of i.i.d. log-normal variables to sum
num_samples = 100000  # Number of samples to generate for estimation
num_bins = 10000  # Number of bins for the histogram

# Generate samples from the original log-normal distribution
original_samples = np.random.lognormal(mean=mu, sigma=sigma, size=num_samples)

# Generate samples for the sum divided by N: create a 2D array of log-normal samples, sum along axis=1, and divide by N
sum_samples_array = np.random.lognormal(mean=mu, sigma=sigma, size=(num_samples, N))
sum_samples = np.sum(sum_samples_array, axis=1) / N

# Determine the common x-axis range based on the combined data
x_min = 0 # min(original_samples.min(), sum_samples.min())
...
axs[1].set_title(f'Sum of {N} i.i.d. log-normals / {N}')
...
plt.tight_layout()
plt.savefig('lognormal_distribution_plot.png', dpi=300)
